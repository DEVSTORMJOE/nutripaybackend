...
router.post("/chat", limiter, async (req, res) => {
  try {
    ...
    // If the request took too long (25s) or timed out from the external API
    if (err.code === 'ECONNABORTED' || err.message.includes('timeout') || err.response?.status === 504 || err.response?.status === 429) {
      return res.status(200).json({ 
        reply: "Nutri AI is currently very busy processing many requests. Please give me a moment and try again shortly!" 
      });
    }
